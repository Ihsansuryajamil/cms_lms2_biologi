@extends('Layouts.app')

@section('sidebar')
    @include('Layouts.sideBarGuru')
@endsection

@section('content')
    <div class="topbar d-flex justify-content-between align-items-center w-100">
        <div class="d-flex align-items-center gap-3">
            <a href="{{ route('guru_course_detail', $topic->course_id) }}" class="btn btn-outline-secondary rounded-pill"><i class="fa-solid fa-arrow-left"></i> Kembali</a>
            <h5 class="mb-0 fw-bold">Tambah Sub-Topik</h5>
        </div>
    </div>

    <div class="content-area">
        <div class="create-form-panel mx-auto bg-white p-5 rounded-4 shadow-sm">
            <div class="create-form-title fs-4 fw-bold mb-1">Materi / Aktivitas Pembelajaran</div>
            <div class="create-form-subtitle text-muted mb-4">Tambahkan aktivitas baru untuk topik: <strong class="text-primary">{{ $topic->nama_topic }}</strong></div>

            @if ($errors->any())
                <div class="alert alert-danger mb-4 rounded-3 border-0">
                    <ul class="mb-0 small">
                        @foreach ($errors->all() as $error)
                            <li>{{ $error }}</li>
                        @endforeach
                    </ul>
                </div>
            @endif

            <form action="{{ route('guru_subtopik_store', $topic->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row g-4">
                    <div class="col-md-12">
                        <label class="form-section-label fw-bold mb-2">Judul Materi / Tugas</label>
                        <input type="text" name="judul" class="form-control border p-3" placeholder="Contoh: Pengenalan Sistem Komputer" value="{{ old('judul') }}" required>
                    </div>

                    <div class="col-md-6">
                        <label class="form-section-label fw-bold mb-2">Tipe Aktivitas</label>
                        <select name="jenis" class="form-select border p-3" required>
                            <option value="materi" {{ old('jenis') == 'materi' ? 'selected' : '' }}>Materi Pembelajaran (PPT, PDF, Video)</option>
                            <option value="tugas" {{ old('jenis') == 'tugas' ? 'selected' : '' }}>Penugasan / Upload File</option>
                            <option value="quiz" {{ old('jenis') == 'quiz' ? 'selected' : '' }}>Kuis / Ujian Interaktif</option>
                        </select>
                    </div>

                    <div class="col-md-6">
                        <label class="form-section-label fw-bold mb-2">Status Publikasi</label>
                        <select name="status" class="form-select border p-3" required>
                            <option value="publish" {{ old('status') == 'publish' ? 'selected' : '' }}>Terbitkan Langsung (Publish)</option>
                            <option value="un-publish" {{ old('status') == 'un-publish' ? 'selected' : '' }}>Simpan sebagai Draft (Un-Publish)</option>
                        </select>
                    </div>

                    <div class="col-12">
                        <label class="form-section-label fw-bold mb-2">Deskripsi / Instruksi Tambahan</label>
                        <textarea name="deskripsi" class="form-control border p-3" rows="4" placeholder="Tuliskan ringkasan materi atau instruksi cara pengerjaan tugas untuk siswa...">{{ old('deskripsi') }}</textarea>
                    </div>

                    <div class="col-md-6">
                        <label class="form-section-label fw-bold mb-2"><i class="fa-solid fa-link me-1"></i> Link Referensi Eksternal</label>
                        
                        <div id="linkContainer">
                            <div class="mb-2" id="linkSlot1">
                                <input type="url" name="link_1" class="form-control border p-3" placeholder="Slot 1: https://..." value="{{ old('link_1') }}">
                            </div>
                            
                            <div class="mb-2 {{ old('link_2') ? '' : 'd-none' }}" id="linkSlot2">
                                <input type="url" name="link_2" class="form-control border p-3" placeholder="Slot 2: https://..." value="{{ old('link_2') }}">
                            </div>
                            
                            <div class="mb-2 {{ old('link_3') ? '' : 'd-none' }}" id="linkSlot3">
                                <input type="url" name="link_3" class="form-control border p-3" placeholder="Slot 3: https://..." value="{{ old('link_3') }}">
                            </div>
                        </div>

                        <button type="button" class="btn btn-light w-100 mt-2 text-primary fw-semibold" id="btnAddLink" style="border: 2px dashed #dee2e6; {{ old('link_3') ? 'display: none;' : '' }}">
                            <i class="fa-solid fa-plus"></i> Tambah Slot Link
                        </button>
                    </div>

                    <div class="col-md-6">
                        <label class="form-section-label fw-bold mb-2"><i class="fa-solid fa-file-arrow-up me-1"></i> Upload File Lampiran</label>
                        
                        <div id="fileContainer">
                            <div class="mb-2" id="fileSlot1">
                                <input type="file" name="file_1" class="form-control border p-2" accept=".pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.zip,.rar">
                            </div>
                            
                            <div class="mb-2 d-none" id="fileSlot2">
                                <input type="file" name="file_2" class="form-control border p-2" accept=".pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.zip,.rar">
                            </div>
                            
                            <div class="mb-2 d-none" id="fileSlot3">
                                <input type="file" name="file_3" class="form-control border p-2" accept=".pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.zip,.rar">
                            </div>
                        </div>
                        
                        <small class="text-muted mt-1 d-block mb-2">Maksimal 10MB per file (PDF, Word, PPT, ZIP).</small>
                        
                        <button type="button" class="btn btn-light w-100 mt-1 text-primary fw-semibold" id="btnAddFile" style="border: 2px dashed #dee2e6;">
                            <i class="fa-solid fa-plus"></i> Tambah Slot File
                        </button>
                    </div>
                </div>
                <div id="quizBuilderArea" class="col-12 mt-4" style="display: none;">
                        <h5 class="fw-bold text-dark border-bottom pb-2 mb-4"><i class="fa-solid fa-layer-group text-primary me-2"></i>Builder Kuis</h5>

                        <div class="mb-4 p-3 bg-light rounded-3 border">
                            <label class="form-section-label fw-bold mb-3 d-block">Pilih Format Kuis yang Ingin Dibuat:</label>
                            <div class="d-flex gap-4">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="format_kuis" id="formatPG" value="pg">
                                    <label class="form-check-label fw-semibold" for="formatPG" style="cursor: pointer;">
                                        <i class="fa-solid fa-list-ul text-primary me-1"></i> Pilihan Ganda (Otomatis Dinilai)
                                    </label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="format_kuis" id="formatEssay" value="essay">
                                    <label class="form-check-label fw-semibold" for="formatEssay" style="cursor: pointer;">
                                        <i class="fa-solid fa-pen-nib text-warning me-1"></i> Soal Essay (Penilaian Manual)
                                    </label>
                                </div>
                            </div>
                        </div>

                        <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4" id="pgCreatedQuizCard" style="display: none;">
                            <div class="card-body p-4">
                                <h6 class="fw-bold text-dark mb-3">
                                    <i class="fa-solid fa-list-ul me-1 text-primary"></i> Tabel Soal Pilihan Ganda
                                    <span class="badge bg-success-subtle text-success ms-2 fw-normal border border-success" style="font-size: 0.7rem;">Nilai Dihitung Otomatis (Max 100 Poin)</span>
                                </h6>
                                <div class="table-responsive">
                                    <table class="table table-bordered table-hover mb-0">
                                        <thead class="table-light">
                                            <tr>
                                                <th style="width: 5%;" class="text-center align-middle">No</th>
                                                <th style="width: 30%;" class="align-middle">Pertanyaan</th>
                                                <th style="width: 12%;" class="text-center align-middle">Opsi A</th>
                                                <th style="width: 12%;" class="text-center align-middle">Opsi B</th>
                                                <th style="width: 12%;" class="text-center align-middle">Opsi C</th>
                                                <th style="width: 12%;" class="text-center align-middle">Opsi D</th>
                                                <th style="width: 10%;" class="text-center align-middle">Jawaban</th>
                                                <th style="width: 7%;" class="text-center align-middle"><i class="fa-solid fa-gear"></i></th>
                                            </tr>
                                        </thead>
                                        <tbody id="pgTableBody">
                                            </tbody>
                                        <tfoot>
                                            <tr>
                                                <td colspan="8" class="text-center py-3">
                                                    <button type="button" id="btnAddPGTable" class="btn btn-primary btn-sm rounded-pill px-4">
                                                        <i class="fa-solid fa-plus me-2"></i> Tambah Pertanyaan PG
                                                    </button>
                                                </td>
                                            </tr>
                                        </tfoot>
                                    </table>
                                </div>
                            </div>
                        </div>

                        <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4" id="essayCreatedQuizCard" style="display: none;">
                            <div class="card-body p-4">
                                <h6 class="fw-bold text-dark mb-3"><i class="fa-solid fa-pen-nib me-1 text-warning"></i> Tabel Soal Essay</h6>
                                <div class="table-responsive">
                                    <table class="table table-bordered table-hover mb-0">
                                        <thead class="table-light">
                                            <tr>
                                                <th style="width: 5%;" class="text-center align-middle">No</th>
                                                <th style="width: 70%;" class="align-middle">Pertanyaan Essay</th>
                                                <th style="width: 15%;" class="text-center align-middle">Bobot Nilai</th>
                                                <th style="width: 10%;" class="text-center align-middle"><i class="fa-solid fa-gear"></i></th>
                                            </tr>
                                        </thead>
                                        <tbody id="essayTableBody">
                                            </tbody>
                                        <tfoot>
                                            <tr>
                                                <td colspan="4" class="text-center py-3">
                                                    <button type="button" id="btnAddEssayTable" class="btn btn-warning text-dark fw-bold btn-sm rounded-pill px-4">
                                                        <i class="fa-solid fa-plus me-2"></i> Tambah Pertanyaan Essay
                                                    </button>
                                                </td>
                                            </tr>
                                        </tfoot>
                                    </table>
                                </div>
                            </div>
                        </div>
                    </div>

                <div class="d-flex justify-content-end gap-3 mt-5 pt-4 border-top">
                    <a href="{{ route('guru_course_detail', $topic->course_id) }}" class="btn btn-light rounded-pill px-4 border py-2 fw-bold text-secondary">Batal</a>
                    <button type="submit" class="btn btn-primary rounded-pill px-5 py-2 fw-bold shadow-sm">
                        <i class="fa-solid fa-save me-1"></i> Simpan Aktivitas
                    </button>
                </div>
            </form>
        </div>
        <!-- <div class="create-form-panel mx-auto bg-white p-5 rounded-4 shadow-sm" id="pgCreatedQuizCard">      
            <div class="card-body p-4">
                <h6 class="fw-bold text-dark mb-3"><i class="fa-solid fa-align-left me-1 text-muted"></i> Tabel Quiz PG</h6>
                
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        <thead class="table-light">
                            <tr>
                                <th style="width: 5%;" class="text-center fw-bold">No</th>
                                <th style="width: 35%;" class="fw-bold">Pertanyaan</th>
                                <th style="width: 12%;" class="text-center fw-bold">A</th>
                                <th style="width: 12%;" class="text-center fw-bold">B</th>
                                <th style="width: 12%;" class="text-center fw-bold">C</th>
                                <th style="width: 12%;" class="text-center fw-bold">D</th>
                                <th style="width: 12%;" class="text-center fw-bold">Jawaban</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td class="text-center align-middle">
                                    <span class="badge bg-light text-dark fw-bold">1</span>
                                </td>
                                <td class="align-middle">
                                    <textarea name="#" class="form-control border p-3" rows="2"></textarea>
                                </td>
                                <td class="text-center align-middle">
                                    <textarea name="#" class="form-control border p-3" rows="2"></textarea>
                                </td>
                                <td class="text-center align-middle">
                                    <textarea name="#" class="form-control border p-3" rows="2"></textarea>
                                </td>
                                <td class="text-center align-middle">
                                    <textarea name="#" class="form-control border p-3" rows="2"></textarea>
                                </td>
                                <td class="text-center align-middle">
                                    <textarea name="#" class="form-control border p-3" rows="2"></textarea>
                                </td>
                                <td class="text-center align-middle">
                                    <select name="#" class="form-select border p-3" required>
                                        <option value="A">A</option>
                                        <option value="B">B</option>
                                        <option value="C">C</option>
                                        <option value="D">D</option>
                                    </select>
                                </td>
                            </tr>
                            <tr>
                                <td class="text-center align-middle">
                                    <span class="badge bg-light text-dark fw-bold">2</span>
                                </td>
                                <td class="align-middle">
                                    <textarea name="#" class="form-control border p-3" rows="2"></textarea>
                                </td>
                                <td class="text-center align-middle">
                                    <textarea name="#" class="form-control border p-3" rows="2"></textarea>
                                </td>
                                <td class="text-center align-middle">
                                    <textarea name="#" class="form-control border p-3" rows="2"></textarea>
                                </td>
                                <td class="text-center align-middle">
                                    <textarea name="#" class="form-control border p-3" rows="2"></textarea>
                                </td>
                                <td class="text-center align-middle">
                                    <textarea name="#" class="form-control border p-3" rows="2"></textarea>
                                </td>
                                <td class="text-center align-middle">
                                    <select name="#" class="form-select border p-3" required>
                                        <option value="A">A</option>
                                        <option value="B">B</option>
                                        <option value="C">C</option>
                                        <option value="D">D</option>
                                    </select>
                                </td>
                            </tr>
                            <tr>
                                <td colspan="7" class="text-center py-3">
                                    <button class="btn btn-primary btn-sm rounded-pill w-100 px-4">
                                        <i class="fa-solid fa-plus me-2"></i> Tambah Pertanyaan
                                    </button>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4" id="essayCreatedQuizCard">
            <div class="card-body p-4">
                <h6 class="fw-bold text-dark mb-3"><i class="fa-solid fa-align-left me-1 text-muted"></i> Tabel Quiz Essay</h6>
                
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        <thead class="table-light">
                            <tr>
                                <th style="width: 5%;" class="text-center fw-bold">No</th>
                                <th style="width: 35%;" class="fw-bold">Pertanyaan</th>
                                <th style="width: 60%;" class="text-right fw-bold">Jawaban Benar</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td class="text-center align-middle">
                                    <span class="badge bg-light text-dark fw-bold">1</span>
                                </td>
                                <td class="text-center align-middle">
                                    <textarea name="#" class="form-control border p-3" rows="2"></textarea>
                                </td>
                                <td class="text-center align-middle">
                                    <textarea name="#" class="form-control border p-3" rows="2"></textarea>
                                </td>
                            </tr>
                            <tr>
                                <td class="text-center align-middle">
                                    <span class="badge bg-light text-dark fw-bold">2</span>
                                </td>
                                <td class="text-center align-middle">
                                    <textarea name="#" class="form-control border p-3" rows="2"></textarea>
                                </td>
                                <td class="text-center align-middle">
                                    <textarea name="#" class="form-control border p-3" rows="2"></textarea>
                                </td>
                            </tr>
                            <tr>
                                <td colspan="7" class="text-center py-3">
                                    <button class="btn btn-primary btn-sm rounded-pill w-100 px-4">
                                        <i class="fa-solid fa-plus me-2"></i> Tambah Pertanyaan
                                    </button>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div> -->
    </div>
    
    <script>
    document.addEventListener('DOMContentLoaded', function() {
        // --- LOGIKA TOGGLE LINK ---
        let linkCount = {{ old('link_3') ? 3 : (old('link_2') ? 2 : 1) }};
        const btnAddLink = document.getElementById('btnAddLink');
        
        btnAddLink.addEventListener('click', function() {
            if (linkCount === 1) {
                document.getElementById('linkSlot2').classList.remove('d-none');
                linkCount++;
            } else if (linkCount === 2) {
                document.getElementById('linkSlot3').classList.remove('d-none');
                linkCount++;
                btnAddLink.style.display = 'none'; // Sembunyikan tombol jika slot penuh
            }
        });

        // --- LOGIKA TOGGLE FILE ---
        let fileCount = 1;
        const btnAddFile = document.getElementById('btnAddFile');
        
        btnAddFile.addEventListener('click', function() {
            if (fileCount === 1) {
                document.getElementById('fileSlot2').classList.remove('d-none');
                fileCount++;
            } else if (fileCount === 2) {
                document.getElementById('fileSlot3').classList.remove('d-none');
                fileCount++;
                btnAddFile.style.display = 'none'; // Sembunyikan tombol jika slot penuh
            }
        });
        // ================= MULAI LOGIKA TABEL KUIS =================
        const jenisSelect = document.querySelector('select[name="jenis"]');
        const quizArea = document.getElementById('quizBuilderArea');
        const pgTableBody = document.getElementById('pgTableBody');
        const essayTableBody = document.getElementById('essayTableBody');
        
        // Element Radio Button & Card
        const radioPG = document.getElementById('formatPG');
        const radioEssay = document.getElementById('formatEssay');
        const pgCard = document.getElementById('pgCreatedQuizCard');
        const essayCard = document.getElementById('essayCreatedQuizCard');

        // 1. Munculkan/Sembunyikan area kuis utama berdasarkan dropdown
        function toggleQuizArea() {
            if (jenisSelect.value === 'quiz') {
                quizArea.style.display = 'block';
                // Reset form pilihan kuis jika ditutup lalu dibuka lagi
                if (!radioPG.checked && !radioEssay.checked) {
                    pgCard.style.display = 'none';
                    essayCard.style.display = 'none';
                }
            } else {
                quizArea.style.display = 'none';
                // Kosongkan tabel agar data tidak terkirim jika tipe diganti
                pgTableBody.innerHTML = '';
                essayTableBody.innerHTML = '';
                radioPG.checked = false;
                radioEssay.checked = false;
            }
        }
        jenisSelect.addEventListener('change', toggleQuizArea);
        toggleQuizArea(); 

        // 2. Munculkan tabel berdasarkan Radio Button Kuis yang dipilih
        function toggleFormatKuis() {
            if (radioPG.checked) {
                pgCard.style.display = 'block';
                essayCard.style.display = 'none';
                essayTableBody.innerHTML = ''; // Kosongkan tabel sebelah jika pindah format
            } else if (radioEssay.checked) {
                essayCard.style.display = 'block';
                pgCard.style.display = 'none';
                pgTableBody.innerHTML = ''; // Kosongkan tabel sebelah jika pindah format
            }
        }
        radioPG.addEventListener('change', toggleFormatKuis);
        radioEssay.addEventListener('change', toggleFormatKuis);

        // 3. Tambah Baris PG Dinamis
        document.getElementById('btnAddPGTable').addEventListener('click', function() {
            const rowCount = pgTableBody.children.length + 1;
            const rowHTML = `
                <tr>
                    <td class="text-center align-middle"><span class="badge bg-light text-dark fw-bold pg-num">${rowCount}</span></td>
                    <td class="align-middle"><textarea name="pg_pertanyaan[]" class="form-control border p-2" rows="2" required></textarea></td>
                    <td class="text-center align-middle"><textarea name="pg_opsi_a[]" class="form-control border p-2" rows="2" required></textarea></td>
                    <td class="text-center align-middle"><textarea name="pg_opsi_b[]" class="form-control border p-2" rows="2" required></textarea></td>
                    <td class="text-center align-middle"><textarea name="pg_opsi_c[]" class="form-control border p-2" rows="2" required></textarea></td>
                    <td class="text-center align-middle"><textarea name="pg_opsi_d[]" class="form-control border p-2" rows="2" required></textarea></td>
                    <td class="text-center align-middle">
                        <select name="pg_kunci[]" class="form-select border p-2" required>
                            <option value="a">A</option>
                            <option value="b">B</option>
                            <option value="c">C</option>
                            <option value="d">D</option>
                        </select>
                    </td>
                    <td class="text-center align-middle">
                        <button type="button" class="btn btn-outline-danger btn-sm btn-remove-row"><i class="fa-solid fa-trash"></i></button>
                    </td>
                </tr>
            `;
            pgTableBody.insertAdjacentHTML('beforeend', rowHTML);
        });

        // 4. Tambah Baris Essay Dinamis
        document.getElementById('btnAddEssayTable').addEventListener('click', function() {
            const rowCount = essayTableBody.children.length + 1;
            const rowHTML = `
                <tr>
                    <td class="text-center align-middle"><span class="badge bg-light text-dark fw-bold essay-num">${rowCount}</span></td>
                    <td class="align-middle"><textarea name="essay_pertanyaan[]" class="form-control border p-2" rows="2" required></textarea></td>
                    <td class="text-center align-middle"><input type="number" name="essay_bobot[]" class="form-control border text-center" value="10" min="1" required></td>
                    <td class="text-center align-middle">
                        <button type="button" class="btn btn-outline-danger btn-sm btn-remove-row"><i class="fa-solid fa-trash"></i></button>
                    </td>
                </tr>
            `;
            essayTableBody.insertAdjacentHTML('beforeend', rowHTML);
        });

        // 5. Hapus Baris dan Update Nomor Urut
        function updateRowNumbers(tbody, numClass) {
            const rows = tbody.querySelectorAll('tr');
            rows.forEach((row, index) => {
                row.querySelector('.' + numClass).textContent = index + 1;
            });
        }

        pgTableBody.addEventListener('click', function(e) {
            if (e.target.closest('.btn-remove-row')) {
                e.target.closest('tr').remove();
                updateRowNumbers(pgTableBody, 'pg-num');
            }
        });

        essayTableBody.addEventListener('click', function(e) {
            if (e.target.closest('.btn-remove-row')) {
                e.target.closest('tr').remove();
                updateRowNumbers(essayTableBody, 'essay-num');
            }
        });
    });
</script>
@endsection